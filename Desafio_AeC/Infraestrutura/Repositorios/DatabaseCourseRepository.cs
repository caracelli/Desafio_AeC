...
namespace Infraestrutura.Repositorios
{
    public class DatabaseCourseRepository : ICourseRepository
    {
        ...
        // Método modificado para retornar o título do curso, ou null se o curso não existir
        public string? GetCourseTitleByLink(string link)
        {
            using (OdbcConnection connection = new OdbcConnection(_odbcConnectionString))
            {
                ...
                using (OdbcCommand command = new OdbcCommand(query, connection))
                {
                    ...
                    return result?.ToString(); // Usando o operador null-conditional (?.)
                }
            }
        }


        // Método modificado para verificar a existência e retornar o título do curso
        public bool Exists(string link, out string cursoTitulo)
        {
            ...
            using (OdbcConnection connection = new OdbcConnection(_odbcConnectionString))
            {
                ...
                using (OdbcCommand command = new OdbcCommand(query, connection))
                {
                    ...
                    if (result != null)
                    {
                        cursoTitulo = result.ToString() ?? string.Empty; // Garantir que a variável nunca seja null
