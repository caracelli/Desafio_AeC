...
namespace Infra
{
    ...
    {
        // String de conexão fixa baseada no caminho relativo
            ...
            try
            {
                using (OdbcConnection connection = new OdbcConnection(_odbcConnectionString))
                {
                    ...
                    using (OdbcCommand command = new OdbcCommand(insertQuery, connection))
                    {
                        ...
                        command.Parameters.Add("?", OdbcType.Text).Value = tipo;
                        command.Parameters.Add("?", OdbcType.Text).Value = evento;
                        command.Parameters.Add("?", OdbcType.Text).Value = status;
